...
def find_support_resistance(levels: list, tolerancia: float = 0.5, min_rebotes: int = 2, ventana: int = 5, max_niveles: int = 3) -> dict:
    ...
    for i in range(ventana, len(levels) - ventana):
        ...
        if (window.index(minimo) == ventana and levels[i] < levels[i-1] and levels[i] < levels[i+1]):
            ...
            if existente:
                existente["rebotes"] += 1; existente["fuerza_total"] += fuerza; existente["ultimos_idx"].append(i)
    ...
    return {"supports": soportes_final, "resistances": resistencias_final, "support": sup_simple, "resistance": res_simple}
...
class UnifiedProbabilitySystem:
    def __init__(self):
        ...
        self.markov_correct: int = 0; self.markov_total: int = 0
        self.ml_correct: int = 0; self.ml_total: int = 0
        self.confidence_factor: float = 0.5; self.volatility: float = 1.0
        self.current_streak: int = 0; self.streak_direction: Optional[str] = None
        self.spins_since_weight_update: int = 0; self.WEIGHT_UPDATE_INTERVAL: int = 50
        self.base_threshold: float = 0.50; self.dynamic_threshold: float = 0.50
        self.ema_trend_factor: float = 1.0; self.sr_factor: float = 1.0

    def calculate_volatility(self, levels: list) -> float:
        if len(levels) < 20: return 1.0
        std_dev = np.std(levels[-20:]); normalized = min(max(std_dev / 5.0, 0.5), 1.5)
        self.volatility = normalized; return normalized
    ...
    def calculate_confidence(self, markov_pred, ml_pred, color: str) -> float:
        ...
        m_prob  = markov_pred.get(color, 0.5); ml_prob = ml_pred.get(color, 0.5)
        agreement = 1.0 - abs(m_prob - ml_prob); self.confidence_factor = 0.4 + agreement * 0.6
        return self.confidence_factor

    def calculate_dynamic_threshold(self) -> float:
        vol_factor = self.volatility; streak_factor = 1.0 + min(self.current_streak * 0.02, 0.3)
        conf_factor = 1.0 - (self.confidence_factor - 0.5) * 0.4
        self.dynamic_threshold = max(0.45, min(0.65, self.base_threshold * vol_factor * streak_factor * conf_factor))
        return self.dynamic_threshold

    def record_prediction(self, color: str, markov_pred, ml_pred, actual: str):
        self.prediction_history.append({"color": color, "markov_pred": markov_pred.get(color, 0.5) if markov_pred else None, "ml_pred": ml_pred.get(color, 0.5) if ml_pred else None, "actual": actual, "timestamp": time.time()})
        if markov_pred is not None:
            self.markov_total += 1
            if (markov_pred.get(color, 0) > 0.5) == (actual == color): self.markov_correct += 1
        if ml_pred is not None:
            self.ml_total += 1
            if (ml_pred.get(color, 0) > 0.5) == (actual == color): self.ml_correct += 1
    ...
    def update_weights(self):
        ...
        markov_acc = self.markov_correct / max(self.markov_total, 1); ml_acc = self.ml_correct / max(self.ml_total, 1)
    ...
    def get_joint_probability(self, category: str, bet_value: str, markov_pred, ml_pred, cat_prob: Optional[float]) -> dict:
        if category == "COLOR":
        ...
        strength = ("strong" if combined_prob >= 0.70 else "moderate" if combined_prob >= 0.60 else "weak")
        return {"combined_prob": combined_prob, "markov_prob": m_p, "ml_prob": ml_p, "signal_strength": strength, "weights": self.weights.copy(), "ema_trend_factor": self.ema_trend_factor, "sr_factor": self.sr_factor, "volatility": self.volatility}

# ─── ESTADÍSTICAS DETALLADAS ─────────────────────────────────────────────────
class DetailedStats:
    def __init__(self):
        self.signal_history: deque = deque(maxlen=50)
        self.wins_1: int = 0; self.wins_2: int = 0; self.losses: int = 0; self.total_signals: int = 0
        self.last_stats_at: int = 0; self.batch_start_bankroll: Optional[float] = None
        self.batch_start_w1: int = 0; self.batch_start_w2: int = 0; self.batch_start_losses: int = 0
        self.last_daily_date = ""; self.daily_start_bankroll: Optional[float] = None
        self.daily_w1: int = 0; self.daily_w2: int = 0; self.daily_losses: int = 0; self.daily_signals: int = 0

    def record_signal_result(self, attempt_won: int, final_result: bool, bet_amount: float, bankroll: float, category: str):
        entry = {"attempt_won": attempt_won, "won": final_result, "bet": bet_amount, "bankroll": bankroll, "timestamp": time.time(), "category": category}
        self.signal_history.append(entry); self.total_signals += 1; self.daily_signals += 1
        if final_result:
    ...
    def mark_stats_sent(self, bankroll: float):
        ...
        self.batch_start_w1 = self.wins_1; self.batch_start_w2 = self.wins_2; self.batch_start_losses = self.losses

    def get_batch_stats(self, current_bankroll: float) -> dict:
        w1 = self.wins_1 - self.batch_start_w1; w2 = self.wins_2 - self.batch_start_w2; l = self.losses - self.batch_start_losses
    ...
    def reset_daily(self, date_str: str, current_bankroll: float):
        self.last_daily_date = date_str; self.daily_start_bankroll = current_bankroll
        self.daily_w1 = 0; self.daily_w2 = 0; self.daily_losses = 0; self.daily_signals = 0

    def reset(self):
        self.signal_history.clear(); self.wins_1 = 0; self.wins_2 = 0; self.losses = 0; self.total_signals = 0
        self.last_stats_at = 0; self.batch_start_bankroll = None; self.reset_daily("", 0.0)
    ...
    for attempt in range(1, _TG_MAX_RETRIES + 1):
        try: return fn(*args, **kwargs)
            ...
            if "retry after" in err.lower():
                ...
                logger.warning(f"Telegram flood-wait {wait}s"); time.sleep(wait); continue
            ...
            else: logger.error(f"Telegram call failed: {e}"); return None
...
class RouletteEngine:
    def __init__(self, name: str, cfg: dict):
        ...
        self.rojo_levels: list = []; self.negro_levels: list = []
        self.par_levels: list = []; self.impar_levels: list = []
        self.alto_levels: list = []; self.bajo_levels: list = []
        ...
        self.signal_active: bool = False; self.waiting_for_attempt: bool = False
        self.waiting_attempt_number: int = 0; self.skip_one_after_zero: bool = False
        self.active_category: Optional[str] = None; self.bet_value: Optional[str] = None
        self.signal_pair: tuple = (); self.bet_color: Optional[str] = None
        ...
        self.no_confirmation_msg_id: Optional[int] = None; self.result_sequence: deque = deque(maxlen=10)
        ...
        self._pending_stats: bool = False
    ...
    def _pretrain_from_db(self, db_path: str, table_name: str):
        ...
        except Exception as e: logger.error(f"[{self.name}] Error leyendo DB: {e}"); return
        if not spins: return
        temp_history = []
        for n in spins:
    ...
    def _persist_spin(self, number: int):
        import time as _time
        try:
            self._live_conn.execute("INSERT INTO live_spins (table_name, number, ts) VALUES (?,?,?)", (self.db_table, number, int(_time.time())))
            ...
            try:
                ...
                self._live_conn.execute("INSERT INTO live_spins (table_name, number, ts) VALUES (?,?,?)", (self.db_table, number, int(_time.time())))
                self._live_conn.commit()
            except Exception as e2: logger.error(f"SQLite irrecuperable: {e2}")

    def _cleanup_old_live_spins(self):
        import time as _time
        try:
            cutoff = int(_time.time()) - 7 * 86400
            self._live_conn.execute("DELETE FROM live_spins WHERE table_name=? AND ts<?", (self.db_table, cutoff))
            self._live_conn.commit()
    ...
    def _blend_prediction(self, category: str, bet_value: str) -> Optional[dict]:
        ...
        if category in ("COLOR", "PARIDAD", "RANGO"):
            if category == "COLOR":
                m_seq = [s["real"] for s in self.spin_history if s["real"]!="VERDE"]; ml_seq = m_seq
                m_pred = self.markov_color.predict(m_seq) if len(m_seq) >= self.markov_color.order else None; ml_pred = self.ml_color.predict(ml_seq)
            elif category == "PARIDAD":
                seq = [s["paridad"] for s in self._paridad_rango_seq if s["paridad"]]
                m_pred = self.markov_paridad.predict(seq) if len(seq) >= self.markov_paridad.order else None; ml_pred = self.ml_paridad.predict(seq)
            else:
                seq = [s["rango"] for s in self._paridad_rango_seq if s["rango"]]
                m_pred = self.markov_rango.predict(seq) if len(seq) >= self.markov_rango.order else None; ml_pred = self.ml_rango.predict(seq)
        ...
        trend_f = self.unified_prob_system.ema_trend_factor; sr_f = self.unified_prob_system.sr_factor
        base_prob = raw_prob * trend_f * sr_f; base_prob = max(0.30, min(0.99, base_prob))
        levels = self._levels_for(category, bet_value); ema_sig = self.amx_system.check_signal(levels, bet_value)
        amx_bonus = 0.03 if (ema_sig and ema_sig.get("strength") == "strong") else 0.0; final_prob = min(0.95, base_prob + amx_bonus)
        return {"combined_prob": final_prob, "markov_prob": markov_prob, "ml_prob": ml_prob, "cat_pattern_prob": cat_prob, "signal_strength": "strong" if final_prob >= 0.70 else "moderate" if final_prob >= 0.60 else "weak", "weights": w if category == "COLOR" else {"markov": None, "ml": None}, "ema_trend_factor": trend_f, "sr_factor": sr_f, "volatility": self.unified_prob_system.volatility}
    ...
    def _evaluate_category(self, category: str) -> Optional[dict]:
        ...
        if category == "DOCENA":
            excl_num = int(best_val[1]); pair = DOZEN_PAIRS[excl_num]; pair_strs = (f"D{pair[0]}", f"D{pair[1]}")
        elif category == "COLUMNA":
            excl_num = int(best_val[1]); pair = COLUMN_PAIRS[excl_num]; pair_strs = (f"C{pair[0]}", f"C{pair[1]}")
        else: pair_strs = ()
        return {"category": category, "bet_value": best_val, "signal_pair": pair_strs, "probability": final_prob, "trigger_number": trigger, "ema_score": ema_sig.get("score", 0), "ema_pattern": ema_sig.get("pattern", ""), "signal_prob_details": unified}
    ...
    def _evaluate_pre_alert(self) -> Optional[dict]:
        ...
        if best_12 is None or best_12_prob < self.min_prob_threshold:
            logger.info(f"[{self.name}] Pre‑alerta: 11° [{best_11['category']}] {best_11['probability']:.1%} → 12° = {best_12_prob:.1%} < 60% ❌")
            return None
        logger.info(f"[{self.name}] ✅ Pre‑alerta: 11° [{best_11['category']}] {best_11['probability']:.1%} → 12° [{best_12['category']}] {best_12_prob:.1%} ≥ 60% ✅")
    ...
    def _simulate_spin12_category(self, cat: str, sim_number: int, sim_real: str) -> Optional[dict]:
        ...
        trend_f = self.unified_prob_system.ema_trend_factor; sr_f = self.unified_prob_system.sr_factor; final = max(0.30, min(0.95, raw * trend_f * sr_f))
        ...
        return {"category": cat, "bet_value": best_val, "signal_pair": signal_pair, "probability": final, "cat_pattern_prob": cat_prob, "markov_prob": markov_prob, "ml_prob": ml_prob, "pattern_total": total_cat, "signal_prob_details": {"combined_prob": final, "markov_prob": markov_prob if markov_prob is not None else 0.5, "ml_prob": ml_prob if ml_prob is not None else 0.5, "cat_pattern_prob": cat_prob, "signal_strength": "strong" if final >= 0.70 else "moderate" if final >= 0.60 else "weak", "weights": self.unified_prob_system.weights.copy(), "ema_trend_factor": trend_f, "sr_factor": sr_f, "volatility": self.unified_prob_system.volatility}}
    ...
    def _markov_predict_simulated(self, cat: str, sim_seq: list, target_val: str) -> Optional[float]:
        order = self.markov_color.order
        if len(sim_seq) < order: return None
        state = tuple(sim_seq[-order:])
        ...
        total = sum(counts.values())
        if total < 8: return None
        return counts.get(target_val, 0) / total

    def _ml_predict_simulated(self, cat: str, sim_seq: list, target_val: str) -> Optional[float]:
        plen = 3
        if len(sim_seq) < plen: return None
        pattern = tuple(sim_seq[-plen:])
    ...
    def _send_pre_alert(self, pre_alert_data: dict):
        ...
        icon_11 = self._category_icon(best_11["bet_value"]); icon_12 = self._category_icon(best_12["bet_value"])
        if best_12.get("signal_pair"):
            p1, p2 = best_12["signal_pair"]; apuesta_12 = f"<b>{p1}</b> {self._category_icon(p1)} y <b>{p2}</b> {self._category_icon(p2)}"
        else: apuesta_12 = f"<b>{best_12['bet_value']}</b> {icon_12}"
        text = (f"⚠️ <b>PRE‑ALERTA</b> ⚠️\n\n🎰 <b>{self.name}</b>\n👁 <i>Esperando confirmación del 11° giro…</i>\n\n📊 <b>Predicción 11°:</b> {best_11['bet_value']} {icon_11} ({best_11['probability']:.0%}) [{best_11['category']}]\n🎯 <b>Señal 12° si acierta:</b> {apuesta_12} ({best_12['probability']:.0%}) [{best_12['category']}]\n\n💡 <i>Se confirmará automáticamente al coincidir el 11°</i>")
    ...
    def _activate_prealert_signal(self, signal_12: Optional[dict], trigger_number: int):
        """Activar señal confirmada desde pre‑alerta para el 12° giro."""
        ...
        unified_prob = signal_12["signal_prob_details"]
        self._send_signal(1, unified_prob); self.amx_system.register_signal_sent()
    ...
    def _send_signal(self, attempt: int, unified_prob: dict):
        if self.pending_prediction and self.pending_prediction.get("pre_alert_msg_id"):
            tg_delete(self.bot, self.chat_id, self.pending_prediction["pre_alert_msg_id"]); self.pending_prediction = None
    ...
    def _process_number_inner(self, number: int):
        ...
        # Actualizar niveles
        if number == 0:
            self.rojo_levels.append(self.rojo_levels[-1] if self.rojo_levels else 0); self.negro_levels.append(self.negro_levels[-1] if self.negro_levels else 0)
            self.par_levels.append(self.par_levels[-1] if self.par_levels else 0); self.impar_levels.append(self.impar_levels[-1] if self.impar_levels else 0)
            self.alto_levels.append(self.alto_levels[-1] if self.alto_levels else 0); self.bajo_levels.append(self.bajo_levels[-1] if self.bajo_levels else 0)
            self.d1_levels.append(self.d1_levels[-1] if self.d1_levels else 0); self.d2_levels.append(self.d2_levels[-1] if self.d2_levels else 0); self.d3_levels.append(self.d3_levels[-1] if self.d3_levels else 0)
            self.c1_levels.append(self.c1_levels[-1] if self.c1_levels else 0); self.c2_levels.append(self.c2_levels[-1] if self.c2_levels else 0); self.c3_levels.append(self.c3_levels[-1] if self.c3_levels else 0)
        else:
        ...
        if self.pending_prediction is not None and not self.signal_active and not self.waiting_for_attempt:
            ...
            cat = pred.get("pre_alert_11", {}).get("category")
            val = pred.get("pre_alert_11", {}).get("bet_value")
            if cat is None or val is None: cat = pred.get("category"); val = pred.get("bet_value") # Fallback
            ...
            if actual_val == val:
                ...
                logger.info(f"[{self.name}] ✅ Pre‑alerta confirmada: 11° giro {cat} {val} – Evaluando señal 12° giro")
                best_12 = self._detect_best_category_signal() # Re-evaluar con dato real
                if best_12 and best_12["probability"] >= self.min_prob_threshold:
                    self._activate_prealert_signal(best_12, number)
                    return
                else: logger.info(f"[{self.name}] Pre‑alerta confirmada pero sin señal >=60% para el 12° giro")
            else:
                if pred.get("pre_alert_msg_id"): tg_delete(self.bot, self.chat_id, pred["pre_alert_msg_id"])
                self.pending_prediction = None
                logger.info(f"[{self.name}] ❌ Pre‑alerta no confirmada para 11° giro ({actual_val} ≠ {val})")
        ...
        if self.signal_active:
            ...
            if result:
                ...
                if self.attempts_left <= 0: level_used = self.bet_sys.level; self._handle_full_loss(number, real, level_used)
                    ...
                    self.bet_sys.loss(); attempt_number = self.total_attempts - self.attempts_left + 1; self.trigger_number = number
                    unified_prob = self._blend_prediction(self.active_category, self.bet_value); self._send_signal(attempt_number, unified_prob)
            ...
            if not best or best["probability"] < self.min_prob_threshold:
                ords = {2:"2°"}; ord_str = ords.get(attempt_number, f"{attempt_number}°")
                if self.no_confirmation_msg_id: tg_delete(self.bot, self.chat_id, self.no_confirmation_msg_id)
                msg = tg_send_text(self.bot, self.chat_id, self.thread_id, f"🔔 Sin confirmación para enviar señal para el intento {ord_str}")
...
async def self_ping_loop():
    ...
    while True:
        try:
            with urllib.request.urlopen(ping_url, timeout=15) as r: logger.info(f"[KeepAlive] 🟢 OK [{r.status}]")
        except Exception as e: logger.warning(f"[KeepAlive] ❌ Falló: {e}")
    ...
    def cmd_status(message):
        ...
        for name, engine in engines.items():
            ...
            if engine.signal_active:
                cat = engine.active_category or "?"; val = engine.bet_value or "?"; icon = CATEGORY_ICONS.get(val, "")
                ...
                if engine.pending_prediction: st = f"🔮 Pre‑alerta pendiente: {engine.pending_prediction.get('pre_alert_11', {}).get('category', '?')} → {engine.pending_prediction.get('pre_alert_11', {}).get('bet_value', '?')}"
                else: st = "⚪ Idle"
...
async def main():
    ...
    def _poll(b: telebot.TeleBot, label: str):
        logger.info(f"Iniciando polling Telegram — {label}"); b.polling(none_stop=True, interval=1, timeout=30)
