#!/usr/bin/env python3
"""
Russian Roulette — Bot DC v34 (AsyncTeleBot + Menú + Canal Secundario)
===========================================================================
- Canal principal: -1003821352139 (pausable con /detenersenal)
- Canal secundario: -1003613599867 (siempre activo)
- Menú de comandos funcional (como Spaceman)
- Estrategias: E1 (docenas), E2 (repetición docena/columna), E3 (retorno), E4 (color), E5 (zona)
- Aprendizaje adaptativo, marcador diario, niveles, repeticiones históricas
"""
...
MAIN_CHAT_ID = -1003821352139           # Canal principal (puede pausarse)
...
# ─── TELEGRAM HELPERS ASÍNCRONOS ──────────────────────────────────────────────
async def tg_send(chat_id: int, text: str, markup: InlineKeyboardMarkup = None):
    if not text:
        return None
    try:
...
async def tg_send_main(text: str, markup: InlineKeyboardMarkup = None):
    if not engine or engine.signals_paused:
        logger.info("📵 Señales pausadas, no se envía al canal principal.")
        return None
    return await tg_send(MAIN_CHAT_ID, text, markup)

async def tg_send_secondary(text: str, markup: InlineKeyboardMarkup = None):
    return await tg_send(SECONDARY_CHAT_ID, text, markup)

async def tg_send_both(text: str, markup: InlineKeyboardMarkup = None):
    await tg_send_secondary(text, markup)
    if engine and not engine.signals_paused:
        await tg_send(MAIN_CHAT_ID, text, markup)
...
# ─── ESTADÍSTICAS Y MARCADOR DIARIO ───────────────────────────────────────────
class DailyStats:
    def __init__(self):
    ...
    def reset_if_new_day(self):
        ...
        if today != self.date:
            ...
            self.losses = 0
            self.net_profit = 0.0
            logger.info("📅 Marcador diario reiniciado automáticamente.")
            asyncio.create_task(tg_send_secondary("📅 *NUEVO DÍA* — El marcador diario ha sido reiniciado automáticamente.", parse_mode="Markdown"))
    ...
    def reset(self):
        ...
        asyncio.create_task(tg_send_secondary("🗑️ *Marcador diario reiniciado manualmente.*", parse_mode="Markdown"))
...
class RussianRouletteEngine:
    ...
    async def _send_signal(self):
        ...
        if not self.signals_paused:
            await tg_send_main(text, markup)

    # ─── Activación y resolución ─────────────────────────────────────────────
    def _activate_signal(self, sig: Dict):
        ...
        asyncio.create_task(self._send_signal())
    ...
    async def _resolve(self, number: int):
        d = get_dozen(number)
        bet_usd = self.gestor.get_bet(self.active_intento)
        ...
        if won:
            ...
            await tg_send_both(result_text, markup=roulette_keyboard())
            ...
            if self.active_intento < MAX_INTENTOS:
                if self.active_signal_msg_id:
                    await tg_delete(MAIN_CHAT_ID, self.active_signal_msg_id)
                    self.active_signal_msg_id = None
                self.active_intento += 1
                ...
                await tg_send_both(loss_text, markup=roulette_keyboard())
    ...
    def _check_stats(self):
        if self.stats.should_send():
            asyncio.create_task(tg_send_both(self.stats.get_stats_text(self.bankroll)))
    ...
    def process_batch(self, batch):
        ...
        for spin in new_spins:
            ...
            if 0 <= number <= 36:
                try:
                    asyncio.create_task(self._process_inner(number))
                except Exception as e:
    ...
    async def _process_inner(self, number: int):
        d = get_dozen(number)
        ...
        if not self.warmup_done:
            ...
            await tg_send_both(
                "🟢 <b>IMMERSIVE ROULETTE DC v34</b> — Sistema listo.\n"
                "5 estrategias · Niveles · Repetición Docena/Columna · Control de señales · Marcador diario"
            )

        if self.signal_active:
            await self._resolve(number)
        else:
            sig = await self._select_best_signal()
            if sig:
...
def home():
    return jsonify({
        "status": "ok", "bot": "Russian Roulette DC v34",
        "strategies": ["E1", "E2(Rep)", "E3", "E4(Color)", "E5(Zona)"],
...
# ─── COMANDOS DE TELEGRAM (asíncronos) ────────────────────────────────────────
@bot.message_handler(commands=['start'])
async def cmd_start(message):
    await bot.reply_to(message,
        "<b>🎰 IMMERSIVE ROULETTE DC v34</b>\n\n"
...
async def cmd_stop_signals(message):
    if message.chat.id != MAIN_CHAT_ID:
        await bot.reply_to(message, "❌ Este comando solo funciona en el canal principal.")
        return
    if engine:
        engine.signals_paused = True
        await bot.reply_to(message, "🔇 *Señales detenidas* — Ya no se enviarán señales al canal principal. El canal secundario sigue activo.", parse_mode="Markdown")
        await tg_send_secondary("🔇 *Señales pausadas* en el canal principal. El bot sigue funcionando en segundo plano.", parse_mode="Markdown")
    else:
        await bot.reply_to(message, "❌ Engine no inicializado.")

@bot.message_handler(commands=['encendersenal'])
async def cmd_start_signals(message):
    if message.chat.id != MAIN_CHAT_ID:
        await bot.reply_to(message, "❌ Este comando solo funciona en el canal principal.")
        return
    if engine:
        engine.signals_paused = False
        await bot.reply_to(message, "🔊 *Señales reanudadas* — El bot volverá a enviar señales al canal principal.", parse_mode="Markdown")
        await tg_send_secondary("🔊 *Señales reanudadas* en el canal principal.", parse_mode="Markdown")
    else:
        await bot.reply_to(message, "❌ Engine no inicializado.")

@bot.message_handler(commands=['resetearmarcador'])
async def cmd_reset_daily(message):
    if message.chat.id != MAIN_CHAT_ID:
        await bot.reply_to(message, "❌ Este comando solo funciona en el canal principal.")
        return
    if engine:
        engine.daily_stats.reset()
        await bot.reply_to(message, "🗑️ *Marcador diario reiniciado.*", parse_mode="Markdown")
        await tg_send_secondary("🗑️ *Marcador diario reiniciado manualmente.*", parse_mode="Markdown")
    else:
        await bot.reply_to(message, "❌ Engine no inicializado.")
...
async def cmd_stats(message):
    if not engine:
        await bot.reply_to(message, "❌ Engine no inicializado")
        return
    await bot.reply_to(message, engine.stats.get_stats_text(engine.bankroll), parse_mode="HTML")

@bot.message_handler(commands=['aprendizaje', 'learning'])
async def cmd_learning(message):
    if not engine:
        await bot.reply_to(message, "❌ Engine no inicializado")
        return
    await bot.reply_to(message, engine.learner.get_summary(30), parse_mode="HTML")

@bot.message_handler(commands=['niveles'])
async def cmd_niveles(message):
    if not engine:
        await bot.reply_to(message, "❌ Engine no inicializado")
        return
    g = engine.gestor
...
async def cmd_debug(message):
    if not engine or not engine.warmup_done:
        await bot.reply_to(message, "⏳ Sistema calentando...", parse_mode="HTML")
        return
    ...
    e4 = await engine._detect_e4()
    e5 = await engine._detect_e5()
    def sig_txt(s):
...
async def cmd_reset_learning(message):
    ...
    try:
        ...
        await bot.reply_to(message, "🗑️ <b>Historial de aprendizaje y repeticiones borrado.</b>", parse_mode="HTML")
    except Exception as e:
        await bot.reply_to(message, f"❌ Error: {e}", parse_mode="HTML")
...
async def main():
    ...
    await bot.set_my_commands([
        types.BotCommand('start', 'Iniciar / estado del bot'),
        types.BotCommand('detenersenal', 'Pausar señales en el canal principal'),
        types.BotCommand('encendersenal', 'Reanudar señales en el canal principal'),
        types.BotCommand('resetearmarcador', 'Reiniciar marcador diario'),
        types.BotCommand('status', 'Estado actual del bot'),
        types.BotCommand('stats', 'Estadísticas totales'),
        types.BotCommand('aprendizaje', 'Resumen de aprendizaje adaptativo'),
        types.BotCommand('niveles', 'Mostrar niveles de apuesta'),
        types.BotCommand('debug', 'Información de depuración'),
        types.BotCommand('reset', 'Reiniciar balance y nivel'),
        types.BotCommand('reset_learning', 'Borrar historial de aprendizaje')
    ])
