...
def fmt_currency_amount(chips: float, currency: str) -> str:
    ...
    if currency in ("USD", "MXN", "PEN"): return f"{sym} {amount:.2f}"
    return f"{sym} {int(round(amount))}"

def fmt_gestion_signal(chips: float) -> str:
    usd = fmt_currency_amount(chips, "USD"); mxn = fmt_currency_amount(chips, "MXN")
    pen = fmt_currency_amount(chips, "PEN"); cop = fmt_currency_amount(chips, "COP")
    ars = fmt_currency_amount(chips, "ARS"); clp = fmt_currency_amount(chips, "CLP")
...
class OnlineEnsemblePredictor:
    ...
    def partial_train(self, hist_c, hist_p, hist_z, target_c, target_p, target_z):
        ...
        for cat, target, mnb, sgd in [
            ...
            y = np.array([target]); classes = getattr(self, f"CLASSES_{cat}")
    ...
    def predict(self, hist_c, hist_p, hist_z, cat: str) -> Optional[dict]:
        ...
        X = np.array(feats).reshape(1, -1)
        try:
            mnb = getattr(self, f"mnb_{cat.lower()}"); sgd = getattr(self, f"sgd_{cat.lower()}")
            nb_p = mnb.predict_proba(X)[0]; sg_p = sgd.predict_proba(X)[0]
            final = 0.5 * nb_p + 0.5 * sg_p; classes = getattr(self, f"CLASSES_{cat.upper()}")
...
class AMXAnalyzer:
    def adjust_probability(self, base_prob: float, target: str, predictions: dict, recent_hist: list, seq_state) -> float:
        ...
        for i in range(1, min(4, len(recent_hist)) + 1):
            if recent_hist[-i] == target: matches += 1
            else: break
        ...
        if target == seq_state.expected(): cross_boost += 0.03
...
class DAlembert:
    BASE_BET = 3
    def __init__(self): self.bet = self.BASE_BET
    def get_bet(self) -> int: return self.bet
    def win(self) -> bool:
        self.bet -= 1
        if self.bet == 0: self.bet = self.BASE_BET; return True
        return False
    def loss(self): self.bet += 1
    def reset(self): self.bet = self.BASE_BET
...
class SequenceState:
    def __init__(self, category: str):
        ...
        self.idx = 0
    def advance(self): self.idx = (self.idx + 1) % len(self.sequence)
    def expected(self) -> str: return self.sequence[self.idx]
    def initialize_from_last_value(self, last_val: str):
        try: idx = self.sequence.index(last_val); self.idx = (idx + 1) % len(self.sequence)
        except ValueError: self.idx = 0

class GlobalStats:
    def __init__(self):
        self.wins = 0; self.zeros = 0; self.losses = 0; self.consecutive = 0
        self.last_20 = deque(maxlen=20); self.signals_processed = 0; self.global_chips: int = 0; self.last_report_signals = 0
    ...
    def record(self, result_type: str, attempt: int, number: int, val, type_str: str, roulette_name: str):
        ...
        elif result_type == 'EMPATE': self.zeros += 1; self.consecutive = 0
        self.last_20.append({"result": result_type, "attempt": attempt, "number": number, "val": val, "type": type_str, "roulette": roulette_name})
    ...
    def get_stats_text(self) -> str:
        ...
        for s in reversed(list(self.last_20)):
            a_str = f"🔄 INTENTO #{s['attempt']}"; b_str = f"💵 +{fmt_currency_amount(s['val'], 'USD')}" if s['result'] == 'WIN' else f"💵 -{fmt_currency_amount(s['val'], 'USD')}"
            if s['result'] == 'WIN': text += f"✅ WIN #{s['number']} {s['type']} | {a_str} | {b_str}\n\n"
            elif s['result'] == 'EMPATE': text += f"🟠 EMPATE #0 ZERO | {a_str} | {b_str}\n\n"
            else: text += f"❌ LOSS #{s['number']} {s['type']} | {a_str} | {b_str}\n\n"
        return text
...
class SessionManager:
    ...
    def seconds_to_next_slot(self) -> float:
        ...
        else: target = (now + timedelta(hours=1)).replace(minute=0, second=0, microsecond=0)
        return max(1.0, (target - now).total_seconds())

    def _start_session(self):
        self.session_active = True; self.session_start_time = time.time(); self.session_start_chips = GLOBAL_STATS.global_chips
    ...
    def _end_session(self):
        engine = self.engines[self.current_idx]; self.session_active = False
        duration_mins = int((time.time() - self.session_start_time) / 60)
        engine.dalembert.reset(); engine.cycle_active = False; engine.signal_active = False; engine.attempt = 1
    ...
    async def session_watchdog(self):
        ...
        while True:
            ...
            if wait > REMINDER_SECS + 10:
                ...
                now = self._now_arg()
                slot_str = now.strftime("%H:30") if now.minute < 30 else (now + timedelta(hours=1)).strftime("%H:00")
                self._reminder_msg_id = tg_send(f"⏰ PRÓXIMA SESIÓN EN 5 MINUTOS ⏰\n🎰 AUTO ROULETTE 🎰\n🕐 Arranca a las {slot_str} hs — ¡prepárate!")
                await asyncio.sleep(REMINDER_SECS)
            else: await asyncio.sleep(wait)
            if self._reminder_msg_id: tg_delete(CHAT_ID, self._reminder_msg_id); self._reminder_msg_id = None
            self._start_session()
            while self.session_active:
                await asyncio.sleep(1)
                if GLOBAL_STATS.global_chips >= self.session_start_chips + SESSION_TARGET: self._end_session(); break
...
class RouletteEngine:
    def __init__(self, ws_key: int, name: str):
        ...
        self.spin_history: list = []; self.hist_color: list = []; self.hist_paridad: list = []; self.hist_zona: list = []
        ...
        self.CLASSES = {"COLOR": ["ROJO", "NEGRO", "CERO"], "PARIDAD": ["PAR", "IMPAR", "CERO"], "ZONA": ["MENOR", "MAYOR", "CERO"]}
        self.seq_states = {cat: SequenceState(cat) for cat in ["COLOR", "PARIDAD", "ZONA"]}
        self.dalembert = DAlembert(); self.attempt = 1; self.cycle_active = False; self.wait_next_spin = False
        self.analyzing_msg_id = None
        self.markov = {cat: SmoothedMarkovPredictor() for cat in ["COLOR", "PARIDAD", "ZONA"]}
        self.ensemble = OnlineEnsemblePredictor(); self.amx = AMXAnalyzer()
        self.signal_active = False; self.active_type = None; self.active_target = ""; self.active_chips = 0
        self._last_signal_prob = 0.0; self.active_signal_msg_id = None
        self.spins_since_train = 0; self.ws_count = 0; self.warmup_done = False
        self._db = self._get_db()
        live = self._load_live_history(); self.ws_count = live; self.warmup_done = live >= WARMUP_SPINS
    ...
    def _update_levels(self, cat: str, val: str):
        pos = POS_VALUE[cat]; neg = NEG_VALUE[cat]
        lv = self.levels[cat]; il = self.inv_levels[cat]
        last_l = lv[-1] if lv else 0; last_il = il[-1] if il else 0
        ...
        lv.append(last_l + ch); il.append(last_il + ich)
        if len(lv) > 300: lv.pop(0); il.pop(0)

    def get_chart_data(self) -> dict:
        out = {}
        for cat in ["COLOR", "PARIDAD", "ZONA"]:
            lv = self.levels[cat][-100:]; il = self.inv_levels[cat][-100:]
            out[cat] = {
                "levels": lv, "inv_levels": il,
    ...
    def _update_state(self, number: int, persist=True, train_model=True):
        ...
        for cat, val in [("COLOR", c), ("PARIDAD", p), ("ZONA", z)]: self._update_levels(cat, val)
        for cat in ["COLOR", "PARIDAD", "ZONA"]: self.seq_states[cat].advance()
        ...
        if train_model and number != 0 and len(self.hist_color) > 5:
            ...
            if self.spins_since_train >= TRAIN_INTERVAL: self._train_models(); self.spins_since_train = 0
    ...
    def _get_predictions(self, cat: str) -> dict:
        hist = getattr(self, f"hist_{cat.lower()}"); classes = self.CLASSES[cat]
    ...
    def detect_signal(self) -> Optional[dict]:
        ...
        for cat in ["COLOR", "PARIDAD", "ZONA"]:
            ...
            for target in targets_map[cat]:
                ...
                if final_prob > best_cat_prob: best_cat_prob = final_prob; best_cat_target = target
            if best_cat_prob > best_prob: best_prob = best_cat_prob; best_info = f"{cat} -> {best_cat_target} ({best_cat_prob*100:.1f}%)"
            if best_cat_prob >= MIN_PROB: valid_signals[cat] = {"target": best_cat_target, "prob": best_cat_prob}

        last_num = self.spin_history[-1]["number"] if self.spin_history else "?"
        logger.info(f"🎲 Giro #{last_num} | Prob Máxima: {best_info} | {'🟢 SEÑAL VALIDA' if valid_signals else '🔴 Sin señal'}")
    ...
    def _build_signal_text(self) -> str:
        ...
        target = self.active_target; target_emoji = EMOJI_MAP.get(target, "")
        target_display = f"{target} {target_emoji}"
        if target == "MENOR": target_display = f"MENOR (1-18) {target_emoji}"
        elif target == "MAYOR": target_display = f"MAYOR (19-36) {target_emoji}"
        elif target == "PAR": target_display = f"PARES {target_emoji}"
        elif target == "IMPAR": target_display = f"IMPARES {target_emoji}"
        gestion = fmt_gestion_signal(self.active_chips)
        return (f"✅ SEÑAL CONFIRMADA — {target_display} ✅\n\n🎰 {self.name}\n👉 ÚLTIMO NÚMERO: {last_num}\n♦️ ENTRAR EN: {target_display}\n"
                f"🔹 INTENTO: {self.attempt} DE 3\n\n💡 PROBABILIDAD PATRÓN — {self._last_signal_prob:.1f}%\n🚨 MONTO DE APUESTA POR PAIS:\n{gestion}")

    def send_signal(self):
        if self.analyzing_msg_id: tg_delete(CHAT_ID, self.analyzing_msg_id); self.analyzing_msg_id = None
        if self.active_signal_msg_id: tg_delete(CHAT_ID, self.active_signal_msg_id)
        msg_id = tg_send_with_button(self._build_signal_text(), self.name)
        if msg_id: self.active_signal_msg_id = msg_id
        queue_broadcast({"type": "signal", "target": self.active_target, "chips": self.active_chips, "attempt": self.attempt, "prob": self._last_signal_prob, "bankroll": GLOBAL_STATS.global_chips})

    def iniciar_senal(self, sig: dict):
        self.cycle_active = True; self.signal_active = True; self.active_type = sig["type"]; self.active_target = sig["target"]
        self._last_signal_prob = sig["prob"] * 100; self.active_chips = sig["chips"]; self.attempt = 1
        self.send_signal()

    def resolve(self, number: int):
        c = COLOR_MAP[number]; p = PARIDAD_MAP[number]; z = ZONA_MAP[number]
        actual_val = c if self.active_type == "COLOR" else (p if self.active_type == "PARIDAD" else z)
        won = (actual_val == self.active_target); is_zero = (number == 0); current_bet = self.active_chips
        
        if won:
            cycle_done = self.dalembert.win(); GLOBAL_STATS.global_chips += current_bet
            ...
            self.wait_next_spin = True; self._send_analyzing_msg(); self._end_cycle()
            queue_broadcast({"type": "result", "result": "WIN", "number": number, "bankroll": GLOBAL_STATS.global_chips})
        else:
            GLOBAL_STATS.global_chips -= current_bet; self.dalembert.loss()
            if self.attempt < 3:
                self.attempt += 1; self.signal_active = False; self.wait_next_spin = True; self._send_analyzing_msg()
    ...
    def _end_cycle(self):
        self.attempt = 1; self.cycle_active = False; self.signal_active = False; self.active_type = None
        self.active_target = ""; self.active_signal_msg_id = None; self._last_signal_prob = 0.0; self.active_chips = 0; self._check_stats()

    def _send_analyzing_msg(self):
        if self.analyzing_msg_id: tg_delete(CHAT_ID, self.analyzing_msg_id)
        self.analyzing_msg_id = tg_send("🚨 ANALIZADO PATRONES EN CADA GIRO 🚨")

    def _check_stats(self):
        if GLOBAL_STATS.should_send(): tg_send(GLOBAL_STATS.get_stats_text()); GLOBAL_STATS.mark_sent()
    ...
    def process_spin(self, number: int, session_active: bool):
        try:
            ...
            if not self.warmup_done:
                self.ws_count += 1
                if self.ws_count >= WARMUP_SPINS: self.warmup_done = True; tg_send("🟢 <b>AUTO ROULETTE 🎰</b> — Sistema Listo.")
                return
            if not session_active: return
            if self.signal_active: self.resolve(number); return
            if self.wait_next_spin: self.wait_next_spin = False; return
            sig = self.detect_signal()
            if not self.signal_active:
                if sig:
                    if self.cycle_active:
                        self.active_type = sig["type"]; self.active_target = sig["target"]; self._last_signal_prob = sig["prob"] * 100
                        self.active_chips = self.dalembert.get_bet(); self.signal_active = True; self.send_signal()
...
async def ws_reader(ws_key: int, engine: RouletteEngine, session_mgr: SessionManager):
    ...
    while True:
        try:
                    ...
                    async for raw in ws:
                        ...
                        if results and isinstance(results, list):
                            if not initial_loaded:
                                ...
                                engine._train_models(); engine.initialize_sequences_from_history()
                            ...
                            if 0 <= n <= 36: engine.process_spin(n, session_mgr.session_active)
                            continue
            ...
            logger.warning(f"[WS-{ws_key}] Desconectado: {e}"); await asyncio.sleep(reconnect_delay); reconnect_delay = min(reconnect_delay * 2, 60)
...
async def ws_client_handler(websocket):
    ...
    try:
        if engines_global:
            e = engines_global[0]
            state = {"type": "init", "bankroll": GLOBAL_STATS.global_chips,
                     "session_active": session_mgr_global.session_active if session_mgr_global else False,
                     "dalembert_bet": e.dalembert.get_bet(), "history": e.spin_history[-20:], "charts": e.get_chart_data()}
            await websocket.send(json.dumps(state))
        while True:
            data = await q.get(); await websocket.send(json.dumps(data))
...
async def _ws_server_main():
    ...
    except Exception as e: logger.error(f"[WSServer] Error al iniciar servidor WS: {e}")


# ─── FLASK ────────────────────────────────────────────────────────────────────
app = Flask(__name__)
engines_global: list = []; session_mgr_global: Optional[SessionManager] = None; SESSION_MANAGER_REF: Optional[SessionManager] = None
...
async def daily_stats_loop():
    while True:
        now_utc = datetime.utcnow(); now_arg = now_utc + timedelta(hours=-3)
        target = now_arg.replace(hour=12, minute=0, second=0, microsecond=0)
        if now_arg >= target: target += timedelta(days=1)
...
def cmd_start(m): bot.reply_to(m, "<b>🎰 AUTO ROULETTE</b>\nSesión ilimitada hasta +$0.80 USD\nD'Alembert Modular — Inicio $0.30\n/status /stats /reset", parse_mode="HTML")

@bot.message_handler(commands=['status'])
def cmd_status(m):
    if not engines_global: return
    e = engines_global[0]; sa = "🟢 Activa" if session_mgr_global and session_mgr_global.session_active else "⚪ Inactiva"
...
def cmd_reset(m):
    ...
    for e in engines_global: e.dalembert.reset(); e._end_cycle()
...
def run_bot():
    ...
    while True:
        try: bot.polling(none_stop=True, interval=1, timeout=30)
            ...
            if "409" in str(e): logger.warning("⚠️ Conflicto 409: Otra instancia del bot está corriendo. Reintentando en 15s..."); time.sleep(15)
            else: logger.error(f"Error en bot.polling: {e}"); time.sleep(5)
...
async def main():
    ...
    session_mgr_global = SessionManager(engines_global); SESSION_MANAGER_REF = session_mgr_global
    threading.Thread(target=run_flask, daemon=True).start()
    threading.Thread(target=run_bot, daemon=True).start()
    await asyncio.sleep(5)
    tasks = [asyncio.create_task(session_mgr_global.session_watchdog()), asyncio.create_task(daily_stats_loop()),
             asyncio.create_task(self_ping_loop()), asyncio.create_task(_ws_server_main())]
    for i, r in enumerate(ROULETTES): tasks.append(asyncio.create_task(ws_reader(r["key"], engines_global[i], session_mgr_global)))
    logger.info(f"[Main] 🎰 Bot Unificado iniciado")
