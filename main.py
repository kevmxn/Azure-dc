...
class DetailedStats:
    ...
    def get_batch_stats(self, current_bankroll: float) -> dict:
        ...
        # Eficiencia por intento
        e_w1 = round(w1 / max(w1 + l, 1) * 100, 2) if (w1 + l) > 0 else 0.0
        e_w2 = round(w2 / max(w2 + l, 1) * 100, 2) if (w2 + l) > 0 else 0.0
        e_w3 = round(w3 / max(w3 + l, 1) * 100, 2) if (w3 + l) > 0 else 0.0
        e_loss = 100.0 if l > 0 else 0.0
...
class RouletteEngine:
    ...
    def _send_signal(self, trigger: int, attempt: int, amx_signal: Optional[dict] = None,
        ...
        """
        Envía señal con el NUEVO FORMATO V21:
        ...
        # Formato nuevo de señal
        after_color = "ROJO" if self.bet_color == "NEGRO" else "NEGRO"
        after_icon = "🔴" if after_color == "ROJO" else "⚫️"
        ...
        if amx_signal:
            ...
            amx_line  = f"{mode_icon} <i>AMX V21 • {amx_signal['mode'].upper()}</i>\n"

        # Caption con nuevo formato
        caption = (
            f"☑️☑️ <b>SEÑAL CONFIRMADA</b> ☑️☑️\n\n"
            f"🎰 <b>Juego: {self.name}</b> 👉 <i>Después de: {trigger} {after_color} {after_icon}</i>\n"
            f"🎯 <b>Apostar a: {self.bet_color}</b> {color_icon}\n\n"
            f"🤖 <b>Probabilidad Unificada: {unified_prob_pct}%</b>\n"
            f"{sys_line}"
    ...
    def _send_retry_signal(self, trigger: int, new_bet: float, attempt_number: int,
                          unified_prob: Optional[dict] = None):
        """
        Envía señal de reintento con el NUEVO FORMATO V21.
        """
        table_prob = int(self.get_prob(trigger, self.bet_color) * 100)
        unified_prob_pct = int(unified_prob["combined_prob"] * 100) if unified_prob else table_prob
        color_icon = "🔴" if self.bet_color == "ROJO" else "⚫️"
        ...
        logger.info(f"[{self.name}] Retry {attempt_number}/{MAX_ATTEMPTS}: "
                    f"{self.bet_color} after {trigger}, bet={new_bet:.2f}, unified_prob={unified_prob_pct}%")
    ...
    def _send_result(self, number: int, real: str, won: bool, bet: float,
        ...
        text = (
            f"🆔 <b>Secuencia:</b> {sequence}\n\n"
            f"{result_icon} <b>RESULTADO:</b> {number} {icon}\n"
            f"💰 <b>Bankroll Actual:</b> {bankroll:.2f} usd\n"
        )
        ...
        result_text = f"{result_icon} <b>Resultado:</b> {number} {icon} — {'Acierto!' if won else 'Fallo'}"
