...
VERSIONES DE CAMBIOS:
  ...
  - 📚 FASE DE APRENDIZAJE: carga 20 giros sin emitir señales
  - A partir del giro 21: análisis y emisión de señales
"""
...
DB_TABLE = "roulette_1"   # tabla Azure en el dump SQL
...
class RouletteEngine:
    def __init__(self, name: str, cfg: dict):
        ...
        # ── Control de fase de aprendizaje (20 giros iniciales) ────────────
        self.learning_phase = True              # True = cargando 20 giros iniciales
        self.learning_spin_count = 0            # Contador de giros cargados
        self.learning_initial_numbers = []       # Almacena los primeros 20 números
        self.max_learning_spins = 20            # Cantidad de giros a cargar sin señales
    ...
    # ─── ACTUALIZACIÓN DE HISTORIAL Y NIVELES ─────────────────────────────────
    def _update_history_and_levels(self, number: int, real: str):
        """
        Actualiza spin_history, result_sequence, original_levels, inverted_levels,
        sistemas AMX y predictores (Markov, ML, CategoryML).
        Usado tanto en fase de aprendizaje como normal.
        """
    ...
    def process_number(self, number: int):
        ...
        # ── FASE DE APRENDIZAJE: Cargar 20 giros sin emitir señales ─────────
        if self.learning_phase:
            self.learning_spin_count += 1
            self.learning_initial_numbers.append(number)
            logger.info(f"[{self.name}] Giro {self.learning_spin_count}/20 cargado: {number} {real}")

            # Actualizar historial, niveles y predictores SIN procesar señales
            self._update_history_and_levels(number, real)

            # Si llegamos a 20 giros, terminamos la fase de aprendizaje
            if self.learning_spin_count >= self.max_learning_spins:
                self.learning_phase = False
                logger.info(f"[{self.name}] Fase de aprendizaje COMPLETADA. Iniciando análisis en giro 21.")
                logger.info(f"[{self.name}] Secuencia cargada: {[n for n in self.learning_initial_numbers[-20:]]}")
            return

        # ── FASE NORMAL: Actualizar datos y procesar señales ──────────────────
        self._update_history_and_levels(number, real)

        # ══════════════════════════════════════════════════════════════════════
        #  MÁQUINA DE ESTADOS (SOLO FUERA DE FASE DE APRENDIZAJE)
        # ══════════════════════════════════════════════════════════════════════
...
<b>FUNCIONAMIENTO:</b>
1️⃣ Pre-entrenamiento con historial de BD
2️⃣ Carga 20 giros del websocket (sin señales)
3️⃣ Gira 21+: análisis y emisión de señales
...
def cmd_status(message):
    ...
    for name, engine in engines.items():
        ...
        # Indicador de fase de aprendizaje
        if engine.learning_phase:
            phase_info = f"📚 Aprendiendo ({engine.learning_spin_count}/{engine.max_learning_spins})"
        elif engine.signal_active:
            cat  = engine.active_category or "?"
            val  = engine.bet_value or "?"
            icon = CATEGORY_ICONS.get(val, "")
            phase_info = f"🟢 [{cat}] {val}{icon} intento {MAX_ATTEMPTS - engine.attempts_left + 1}/{MAX_ATTEMPTS}"
        elif engine.waiting_for_attempt:
            phase_info = f"⏳ Esperando intento {engine.waiting_attempt_number}/{MAX_ATTEMPTS}"
        else:
            phase_info = "⚪ Idle"

        w = engine.unified_prob_system.weights
        lines.append(f"<b>{name}</b>: {mode_icon} — {phase_info} [M:{w['markov']:.2f} ML:{w['ml']:.2f}]")
