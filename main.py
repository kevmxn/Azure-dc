#!/usr/bin/env python3
"""
Roulette Telegram Signal Bot - Sistema AMX V23.2
CAMBIOS V23.2 sobre V23.1:
  - Retry 2/3 mejorado con evaluación entre categorías y penalización por cambio.
  - Umbrales ajustados: I2=0.54, I3=0.57 (menos restrictivo pero controlado).
  - Margen de acuerdo ML-Markov reducido a 4% (desde 5%).
  - Momentum unificado: intentos 2 y 3 requieren al menos 1 acierto reciente.
  - Cooldown de 30 segundos entre señales para reducir volatilidad.
  - Penalización del 5% al cambiar de categoría en reintentos.
"""
...
# --- Parámetros ajustados V23.2 (menos restrictivos pero controlados) ---
RETRY_THRESHOLD  = {1: 0.54, 2: 0.54, 3: 0.57}
MIN_SAMPLES      = {1: 10,   2: 9,    3: 8}      # ligeramente más conservador
RETRY_MIN_RECENT_ACC = 0.525   # reducido de 0.55
ANTI_BOUNCE_N    = 3          # se mantiene
MOMENTUM_REQ     = {2: 1, 3: 1}   # ambos requieren al menos 1 acierto reciente
CROSS_CATEGORY_PENALTY = 0.05     # penalización por cambiar de categoría en reintentos
COOLDOWN_SECONDS = 0         # cooldown entre señales para evitar volatilidad
...
@app.route("/")
def index(): return jsonify({"status":"ok","bot":"AMX V23.2","ts":time.time()})
@app.route("/ping")
...
<b>🎰 Roulette Bot - Sistema AMX V23.2</b>
...
async def main():
    ...
    logger.info("🎰 AMX V23.2 iniciado")
