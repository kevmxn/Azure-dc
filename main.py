#!/usr/bin/env python3
"""
Roulette Telegram Signal Bot - Sistema AMX V23.1
CAMBIOS V23.1 sobre V23:
  - Retry 2/3 mejorado: anti-bounce filter, momentum confirmation,
    historial reciente por categoría, margen de acuerdo ML-Markov ≥5%.
  - Resultado muestra emoji de CATEGORÍA: PAR🟣, IMPAR🟡, BAJO🟤, ALTO🔵, CERO🟢
  - Gráfico por categoría: niveles acumulados propios para PARIDAD y RANGO.
"""
...
RETRY_THRESHOLD  = {1: 0.54, 2: 0.57, 3: 0.61}
MIN_SAMPLES      = {1: 10,   2: 6,    3: 5}
RETRY_MIN_RECENT_ACC = 0.55   # historial reciente de aciertos mínimo para reintentos
ANTI_BOUNCE_N    = 3          # ventana anti-bounce
...
class AMXSignalSystem:
    def __init__(self, mode: Literal["tendencia","moderado"]="moderado"):
        self.mode=mode; self.last_signal_time=0.0; self.cooldown_seconds=0
...
class RouletteEngine:
    ...
    def _score_category(self, category, bet_value, attempt=1) -> float:
        ...
        # Margen de acuerdo ≥5% obligatorio en reintentos
        if attempt>1 and abs(ml_prob-mk_prob)<0.05: return 0.0
    ...
    # ─── REINTENTO MEJORADO ───────────────────────────────────────────────────
    def _best_retry_value(self, trigger_number, attempt) -> Optional[str]:
        thr=RETRY_THRESHOLD.get(attempt,0.57); cat=self.active_category
        if   cat=="COLOR":   candidates=[self.bet_value, self._opposite_color(self.bet_value)]
        elif cat=="PARIDAD": candidates=["PAR","IMPAR"]
        else:                candidates=["BAJO","ALTO"]
        best_val=None; best_score=0.0
        for val in candidates:
            # 1. Anti-bounce
            if not self._passes_anti_bounce(cat,val,n=ANTI_BOUNCE_N):
                logger.info(f"[{self.name}] Anti-bounce bloqueó {val}"); continue
            # 2. Momentum (intento 2: ≥1, intento 3: ≥2)
            req=1 if attempt==2 else 2
            if not self._has_momentum(cat,val,require=req):
                logger.info(f"[{self.name}] Sin momentum {val} (need {req})"); continue
            # 3. Historial reciente de aciertos
            acc=self._recent_acc(cat,val,last_n=10)
            if acc<RETRY_MIN_RECENT_ACC:
                logger.info(f"[{self.name}] Acc reciente {acc:.0%}<{RETRY_MIN_RECENT_ACC:.0%}"); continue
            # 4. Score (incluye margen acuerdo ≥5%)
            score=self._score_category(cat,val,attempt=attempt)
            if score>=thr and score>best_score: best_val,best_score=val,score
        if best_val: logger.info(f"[{self.name}] Retry {attempt}: {cat} {best_val} score={best_score:.3f}")
        return best_val
    ...
    def _send_result(self, number, real, won, bet, attempt_won):
        ...
        if self.waiting_msg_id: tg_delete(self.chat_id,self.waiting_msg_id); self.waiting_msg_id=None
        chart=self._get_chart()
        text=f"{'✅' if won else '❌'} Resultado: {rd} — {'Acierto!' if won else 'Fallo'} 💰 {bk:.2f} usd"
        ...
        if self.signal_active:
            ...
            if is_win:
                ...
                if self.attempts_left<=0:
                    ...
                    if chosen:
                        self.bet_value=chosen; self.trigger_number=number
                        up=self._get_cat_prob(self.active_category,chosen,number)
...
@app.route("/")
def index(): return jsonify({"status":"ok","bot":"AMX V23.1","ts":time.time()})
@app.route("/ping")
...
<b>🎰 Roulette Bot - Sistema AMX V23.1</b>

<b>Mejoras en Retry (intento 2/3):</b>
• <b>Anti-bounce</b>: bloquea si los últimos 3 van en contra del valor
• <b>Momentum</b>: ≥1 coincidencia reciente (int.2), ≥2 (int.3)
• <b>Acc. reciente</b>: historial por categoría ≥55% para habilitar retry
• <b>Margen acuerdo</b>: ML y Markov deben diferir ≥5% (no ruido)
• <b>Thresholds</b>: I1=0.54, I2=0.57, I3=0.61
...
<b>Gráfico propio por categoría</b> (niveles acumulados separados)
...
async def main():
    ...
    logger.info("🎰 AMX V23.1 iniciado")
