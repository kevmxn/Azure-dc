#!/usr/bin/env python3
"""
Roulette Telegram Signal Bot - Sistema AMX V24.1
CAMBIOS V24.1 (Entrenamiento desde Base de Datos):
  - Carga incremental de datos desde SQLite
  - Pre-entrenamiento de TCN, LSTM, Transformer con historial completo
  - Batch training para grandes volúmenes de datos
...
DB_TABLE = "roulette_1"
MODEL_CACHE_PATH = "model_cache.pkl"  # Cache de modelos entrenados
...
# ─── Parámetros V24.1 (Entrenamiento desde DB) ──────────────────────────────
...
DB_BATCH_SIZE = 1000  # Tamaño de batch para carga desde DB
MAX_TRAINING_SAMPLES = 5000  # Máximo de muestras para entrenamiento
...
# ═══════════════════════════════════════════════════════════════════════════════
# ═══ MODELOS ML AVANZADOS CON ENTRENAMIENTO DESDE DB ═══════════════════════════
# ═══════════════════════════════════════════════════════════════════════════════

class DatabaseTrainer:
    """
    Gestiona el entrenamiento de modelos desde la base de datos SQLite
    """
    def __init__(self, db_path: str, table_name: str):
        self.db_path = db_path
        self.table_name = table_name
        self.cache_path = MODEL_CACHE_PATH
    ...
    def connect(self):
        ...
        try:
            ...
            logger.error(f"[DBTrainer] Error conectando a DB: {e}")
            return False
    ...
    def get_spin_count(self) -> int:
        """Obtener cantidad total de giros en la base de datos"""
        ...
        try:
            ...
            logger.warning(f"[DBTrainer] Error contando giros: {e}")
            return 0
    
    def load_spins_batch(self, offset: int = 0, limit: int = DB_BATCH_SIZE) -> List[Dict]:
        """
        Cargar giros en batches para entrenamiento eficiente
        """
        if not self.connection:
            return []
        ...
        try:
            ...
            logger.warning(f"[DBTrainer] Error cargando batch: {e}")
            return []
    
    def load_all_spins(self, max_samples: int = MAX_TRAINING_SAMPLES) -> List[Dict]:
        """
        Cargar todos los giros para entrenamiento inicial
        """
        ...
        # Calcular offset para obtener las muestras más recientes si hay muchas
        total_count = self.get_spin_count()
        if total_count > max_samples:
            offset = total_count - max_samples
            logger.info(f"[DBTrainer] Cargando últimos {max_samples} de {total_count} giros")
        else:
            logger.info(f"[DBTrainer] Cargando {total_count} giros totales")
        
        while True:
            batch = self.load_spins_batch(offset, DB_BATCH_SIZE)
            if not batch:
                break
            spins.extend(batch)
            offset += len(batch)
            
            if len(spins) >= max_samples:
                spins = spins[-max_samples:]
                break
        
        return spins
    
    def get_db_hash(self) -> str:
        """Obtener hash de la base de datos para invalidar caché"""
        try:
            # Hash basado en tamaño y fecha de modificación
    ...
    def save_model_cache(self, models_data: Dict):
        """Guardar modelos entrenados en caché"""
        try:
            cache = {
            ...
            with open(self.cache_path, 'wb') as f:
                pickle.dump(cache, f)
            logger.info(f"[DBTrainer] Modelos guardados en caché")
        except Exception as e:
            logger.warning(f"[DBTrainer] Error guardando caché: {e}")
    
    def load_model_cache(self) -> Optional[Dict]:
        """Cargar modelos desde caché si son válidos"""
        try:
            ...
            if cache.get('db_hash') != current_hash:
                logger.info(f"[DBTrainer] Caché inválido (DB modificada)")
                return None
            
            age = time.time() - cache.get('timestamp', 0)
            if age > 86400:  # 24 horas
                logger.info(f"[DBTrainer] Caché expirado ({age/3600:.1f}h)")
                return None
            
            logger.info(f"[DBTrainer] Modelos cargados desde caché")
            return cache.get('models')
            
        except Exception as e:
            logger.warning(f"[DBTrainer] Error cargando caché: {e}")
            return None
...
class TemporalConvolutionalNetwork:
    ...
    def train_from_history(self, spins: List[Dict], epochs: int = 3):
        """
        Entrenar el modelo con historial completo desde la base de datos
        """
        if len(spins) < self.pattern_length * max(self.dilations) + 10:
            logger.warning(f"[TCN] Insuficientes datos para entrenar: {len(spins)}")
            return
        
        logger.info(f"[TCN] Entrenando con {len(spins)} giros...")
        
        # Codificar colores
        encoded = [1.0 if s['real'] == "ROJO" else (0.0 if s['real'] == "NEGRO" else 0.5) 
                   for s in spins]
        
        # Entrenar múltiples épocas
        for epoch in range(epochs):
            errors = []
            # Deslizar ventana de entrenamiento
            window_size = self.pattern_length * max(self.dilations) + 1
            
            for i in range(window_size, len(encoded)):
                seq = np.array(encoded[i-window_size:i])
                target = encoded[i]
                
                # Forward pass para cada dilatación
                for dilation in self.dilations:
                    pred = self._causal_convolution(seq, dilation)
                    error = target - pred
                    errors.append(abs(error))
                    
                    # Actualizar pesos (online learning)
                    dilated_seq = seq[::dilation][-self.pattern_length:]
                    if len(dilated_seq) == self.pattern_length:
                        lr = 0.01 * (1.0 - epoch * 0.2)  # Learning rate decay
                        self.filters[dilation]['weights'] += lr * error * dilated_seq
                        self.filters[dilation]['bias'] += lr * error
            
            mae = np.mean(errors)
            logger.info(f"[TCN] Época {epoch+1}/{epochs} - MAE: {mae:.4f}")
        
        logger.info(f"[TCN] Entrenamiento completado")
    ...
    def get_state(self) -> Dict:
        """Obtener estado para guardar en caché"""
        return {
            'filters': {k: {
        ...
        if 'filters' in state:
            for k, v in state['filters'].items():
                if k in self.filters:
                    self.filters[k]['weights'] = v['weights'].copy()
                    self.filters[k]['bias'] = v['bias']
        if 'history' in state:
...
class LSTMAttentionPredictor:
    ...
    def __init__(self, hidden_size: int = 16, sequence_length: int = 20):
        ...
        self.W_f = np.random.randn(hidden_size, hidden_size + 1) * 0.1
        self.W_i = np.random.randn(hidden_size, hidden_size + 1) * 0.1
        self.W_c = np.random.randn(hidden_size, hidden_size + 1) * 0.1
        self.W_o = np.random.randn(hidden_size, hidden_size + 1) * 0.1
    ...
    def train_from_history(self, spins: List[Dict], epochs: int = 3):
        """
        Entrenar LSTM con historial completo desde la base de datos
        """
        if len(spins) < self.sequence_length + 10:
            logger.warning(f"[LSTM] Insuficientes datos: {len(spins)}")
            return
        
        logger.info(f"[LSTM] Entrenando con {len(spins)} giros...")
        ...
        for epoch in range(epochs):
            ...
            for i in range(self.sequence_length, len(encoded)):
                seq = encoded[i-self.sequence_length:i]
                target = encoded[i]
                ...
                # Predicción
                pred = self._sigmoid(np.mean(h))
                error = target - pred
                errors.append(abs(error))
                
                # Backprop simplificado (solo capa de salida)
                lr = 0.001 * (1.0 - epoch * 0.2)
                self.W_o += lr * error * np.random.randn(*self.W_o.shape) * 0.01
            
            mae = np.mean(errors)
            logger.info(f"[LSTM] Época {epoch+1}/{epochs} - MAE: {mae:.4f}")
        
        logger.info(f"[LSTM] Entrenamiento completado")
    ...
    def _attention(self, query: np.ndarray, keys: List[np.ndarray], values: List[np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def get_state(self) -> Dict:
        ...
        return {
            'W_f': self.W_f.copy(), 'W_i': self.W_i.copy(),
            'W_c': self.W_c.copy(), 'W_o': self.W_o.copy(),
            'W_q': self.W_q.copy(), 'W_k': self.W_k.copy(), 'W_v': self.W_v.copy(),
            'history': list(self.history)
        }
    
    def set_state(self, state: Dict):
        """Restaurar estado"""
        for key in ['W_f', 'W_i', 'W_c', 'W_o', 'W_q', 'W_k', 'W_v']:
            if key in state:
...
class TransformerTimeSeries:
    ...
    def train_from_history(self, spins: List[Dict], epochs: int = 3):
        """
        Entrenar Transformer con historial completo desde la base de datos
        """
        if len(spins) < self.sequence_length + 10:
            logger.warning(f"[Transformer] Insuficientes datos: {len(spins)}")
            return
        
        logger.info(f"[Transformer] Entrenando con {len(spins)} giros...")
        ...
        for epoch in range(epochs):
            ...
            for i in range(self.sequence_length, len(encoded)):
                seq = np.array(encoded[i-self.sequence_length:i])
                ...
                attn_output, _ = self._multi_head_attention(X)
                X = X + attn_output
                
                # Predicción
                last_token = X[-1]
                pred = self._sigmoid(np.mean(last_token))
                ...
                # Actualizar pesos
                lr = 0.0001 * (1.0 - epoch * 0.2)
                self.W_o += lr * error * np.random.randn(*self.W_o.shape) * 0.01
            
            mae = np.mean(errors)
            logger.info(f"[Transformer] Época {epoch+1}/{epochs} - MAE: {mae:.4f}")
        
        logger.info(f"[Transformer] Entrenamiento completado")
    ...
    def get_state(self) -> Dict:
        """Obtener estado para guardar"""
        return {
            'W_q': [w.copy() for w in self.W_q],
            'W_k': [w.copy() for w in self.W_k],
            'W_v': [w.copy() for w in self.W_v],
            'W_o': self.W_o.copy(),
            'history': list(self.history)
        }
    ...
    def set_state(self, state: Dict):
        """Restaurar estado"""
        if 'W_q' in state:
            self.W_q = [w.copy() for w in state['W_q']]
        if 'W_k' in state:
            self.W_k = [w.copy() for w in state['W_k']]
        if 'W_v' in state:
            self.W_v = [w.copy() for w in state['W_v']]
        if 'W_o' in state:
...
class DynamicEnsemble:
    ...
    def __init__(self):
        ...
        # Trainer de base de datos
        self.db_trainer = DatabaseTrainer(DB_PATH, DB_TABLE)
    ...
    def train_from_database(self, force_retrain: bool = False):
        """
        Entrenar todos los modelos desde la base de datos
        """
        logger.info("[Ensemble] Iniciando entrenamiento desde base de datos...")
        
        # Intentar cargar desde caché
        if not force_retrain:
            cached = self.db_trainer.load_model_cache()
            if cached:
                self._restore_from_cache(cached)
                return
        
        # Conectar a DB
        if not self.db_trainer.connect():
            logger.error("[Ensemble] No se pudo conectar a la base de datos")
            return
        
        try:
            # Cargar datos
            spins = self.db_trainer.load_all_spins(MAX_TRAINING_SAMPLES)
            
            if len(spins) < 100:
                logger.warning(f"[Ensemble] Insuficientes datos para entrenar: {len(spins)}")
                return
            
            # Entrenar modelos neuronales
            logger.info("[Ensemble] Entrenando modelos neuronales...")
            self.models['tcn'].train_from_history(spins, epochs=3)
            self.models['lstm_att'].train_from_history(spins, epochs=3)
            self.models['transformer'].train_from_history(spins, epochs=3)
            
            # Guardar en caché
            self._save_to_cache()
            
            logger.info(f"[Ensemble] Entrenamiento completado con {len(spins)} giros")
            
        finally:
            self.db_trainer.disconnect()
    
    def _save_to_cache(self):
        """Guardar estado de modelos en caché"""
        models_data = {
            'tcn': self.models['tcn'].get_state(),
            'lstm_att': self.models['lstm_att'].get_state(),
            'transformer': self.models['transformer'].get_state(),
            'weights': self.weights.copy(),
            'performance': {k: list(v) for k, v in self.performance_history.items()}
        }
        self.db_trainer.save_model_cache(models_data)
    
    def _restore_from_cache(self, cached: Dict):
        """Restaurar modelos desde caché"""
        try:
            if 'tcn' in cached:
                self.models['tcn'].set_state(cached['tcn'])
            if 'lstm_att' in cached:
                self.models['lstm_att'].set_state(cached['lstm_att'])
            if 'transformer' in cached:
                self.models['transformer'].set_state(cached['transformer'])
            if 'weights' in cached:
                self.weights = cached['weights']
            if 'performance' in cached:
                for k, v in cached['performance'].items():
                    self.performance_history[k] = deque(v, maxlen=50)
            logger.info("[Ensemble] Modelos restaurados desde caché")
        except Exception as e:
            logger.error(f"[Ensemble] Error restaurando caché: {e}")
    ...
    def predict(self, spin_history: list, category: str = "COLOR", 
        ...
        consistency = 1.0 - abs(majority_vote - 0.5) * 2
...
class RouletteEngine:
    def __init__(self, name: str, cfg: dict):
        ...
        # NUEVO: Ensemble ML Avanzado con entrenamiento desde DB
        ...
        self.ensemble_ml.train_from_database()
    ...
    def _eval_color(self) -> Optional[dict]:
        ...
        for color in ["ROJO", "NEGRO"]:
            ...
            w_ml = 0.65; w_mk = 0.35
            scores[color] = w_ml * prob + w_mk * mk_prob
    ...
    def _best_retry_value(self, trigger_number, attempt) -> Optional[dict]:
        ...
        anti_bounce_n = ANTI_BOUNCE_N if attempt == 2 else 2
            ...
            for val in candidates:
                ...
                if attempt == 3:
                    ...
                    if not self._has_momentum(cat, val, require=req):
                        continue
                acc = self._recent_acc(cat, val, last_n=8 if attempt == 3 else 10)
                if acc < RETRY_MIN_RECENT_ACC:
                    continue
                score = self._score_category(cat, val, attempt=attempt) - penalty
                min_acceptable = thr - (0.02 if attempt == 3 else 0.0)
...
@app.route("/")
def index(): return jsonify({"status":"ok","bot":"AMX V24.1","ts":time.time()})
@app.route("/ping")
...
<b>🎰 Roulette Bot - Sistema AMX V24.1</b>

<b>🧠 Ensemble ML Avanzado con Entrenamiento desde DB:</b>
• <b>TCN</b>: Redes convolucionales temporales
...
<b>📊 Entrenamiento desde Base de Datos:</b>
• Carga incremental de hasta 5000 giros históricos
• Pre-entrenamiento de modelos neuronales
• Caché de modelos para inicio rápido
• Actualización online continua

/retrain - Forzar re-entrenamiento desde DB
