...
Incluye evaluación automática de color opuesto (sin notificación visual).
"""
...
class RouletteEngine:
    ...
    # --------------------------------------------------------------------------
    # EVALUACIÓN DE INVERSIÓN DE SEÑAL (SIN NOTIFICACIÓN AL USUARIO)
    # --------------------------------------------------------------------------
    def _evaluate_inversion(self, expected_color: str, trigger_number: int) -> tuple[str, float, str]:
        """
        Decide si es mejor apostar al color opuesto basado en:
        - Rachas de pérdidas del color esperado
        - Comparación de probabilidades ML/Markov
        - Condiciones técnicas del color contrario
        Retorna (color_final, probabilidad_ml_asociada, motivo_log)
        No se envía ningún mensaje al usuario sobre la inversión.
        """
        opposite = "NEGRO" if expected_color == "ROJO" else "ROJO"

        # 1. Obtener features para el color opuesto
        ...
        # 2. Obtener features para el color original
        ...
        # 3. Condiciones técnicas del color opuesto
        ...
        # 4. Racha de pérdidas en el color esperado (últimas 3 señales)
    ...
    # --------------------------------------------------------------------------
    # PROCESAMIENTO DE NÚMEROS (INTEGRACIÓN DE INVERSIÓN)
    # --------------------------------------------------------------------------
    def process_number(self, number: int):
        ...
        # Si hay señal activa y ya pasó el tiempo de resultado
        if self.signal_active and time.time() > self.result_until:
            ...
            # Registrar en historial de señales
            ...
            if is_win:
                ...
                if self.attempts_left <= 0:
                    ...
                    logger.info(f"[{self.name}] Retry ML check: {reason}")
                    if emit:
        ...
        if not self.signal_active and time.time() > self.result_until:
            ...
            if not signal:
                ...
                if expected_classic:
                    ...
                    # Para la clásica, la inversión se evaluará sobre bet_color_base
                else:
                    return  # sin señal
            ...
            if signal:
                ...
                # 2. Evaluar si conviene invertir el color
                final_color, _, inv_reason = self._evaluate_inversion(base_color, trigger_num)

                # 3. Construir features con el color final
                ...
                if emit:
                    ...
                    self.expected_color  = base_color   # color esperado original (para historial)
                    self.bet_color       = final_color  # color real de apuesta
